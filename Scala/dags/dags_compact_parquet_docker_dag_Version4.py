...
COMPOSE_NETWORK = os.environ.get("COMPOSE_NETWORK", "scala_default")  # docker-compose network name
...
# Если нужно временно оставить контейнеры для отладки, установите KEEP_CONTAINERS=1 в env Airflow
...
with DAG(
    ...
    schedule_interval="0 2 * * *",  # пример: ежедневно в 02:00 UTC (подставьте нужное расписание по ТЗ)
    catchup=False,
    max_active_runs=1,
    tags=["docker", "spark"],
) as dag:

    # Общая фабрика command / wrapper (использует /bin/sh -c и set -eu)
    def make_command(mode: str, extra_args: str = "") -> str:
        ...
        # wrapper без pipefail (используется /bin/sh в образе)
        wrapper = (
            "set -eu; "
            "echo '=== DIAGNOSTIC START ==='; "
            "echo 'ls /data before:'; ls -la /data || true; "
            f"echo 'Running: {spark_submit}'; {spark_submit}; "
            "echo 'ls /data after:'; ls -la /data || true; "
            "echo '=== DIAGNOSTIC END ===';"
        )
        return wrapper
    ...
    generate = DockerOperator(
        ...
        network_mode=COMPOSE_NETWORK,      # важно, чтобы контейнер видел postgres по имени
        mounts=mounts,
        auto_remove=(not KEEP_CONTAINERS), # по умолчанию удаляем контейнеры; для отладки можно установить KEEP_CONTAINERS=1
        mount_tmp_dir=False,
        tty=False,
        environment={
