...
with DAG(
    ...
    prep = DockerOperator(
        ...
        image="alpine:3.20",
        command=["/bin/sh", "-lc", "mkdir -p /data/parquet && chown -R 50000:0 /data && chmod -R g+rwX /data && ls -ld /data/parquet"],
    ...
    generate = DockerOperator(
        ...
        image=JOB_IMAGE,
        api_version="auto",
        command=["/bin/sh", "-lc", make_wrapper("generate")],
        docker_url="unix://var/run/docker.sock",
        network_mode=COMPOSE_NETWORK,
        mounts=mounts,
        mount_tmp_dir=False,
    ...
    compact = DockerOperator(
        ...
        image=JOB_IMAGE,
        api_version="auto",
        command=["/bin/sh", "-lc", make_wrapper("compact")],
        docker_url="unix://var/run/docker.sock",
        network_mode=COMPOSE_NETWORK,
        mounts=mounts,
        mount_tmp_dir=False,
